...
add_tile(char type)
{
	...
	case 'v':
		tiles_bank[i].ID = i;
		tiles_bank[i].type = '|';
		tiles_bank[i].cart = add_cart(type);
		break;
	case '^':
		tiles_bank[i].ID = i;
		tiles_bank[i].type = '|';
...
parse_input(tTrack * pTrack)
{
	...
	tTile ** scan;
	tCart * new_cart;
	tCartList * pList = &pTrack -> cart_list;
		...
		else if(ch != ' ')
		{
			*scan = add_tile(ch);
			...
			if(new_cart)
			{
				new_cart -> cor.x = col;
				new_cart -> cor.y = row;
				pList -> list[pList -> size] = new_cart;
				++pList -> size;
			}
			/*
	...
	for(int i = 0; i < TRACK_SIZE; ++i)
	{
		...
		for(int j = 0; j < TRACK_SIZE; ++j)
		{
			...
			if(scan)
			{
				empty = false;
				if(scan -> cart)
					putchar(cart_dir(scan -> cart -> dir));
				else
					putchar(scan -> type);
			}
			else
...
destination(tTrack * pTrack, tCart * pCart)
{
	...
	{
	/*	
		printf("ID:%u, y%d, x%d, dir:%u, tile:%c\n",
			pCart -> ID, y, x, pCart -> dir,
...
void
remove_cart(tCartList * pList, tCart * pCart)
{
	for(int i = 0; i < pList -> size; ++i)
	{
		if(pList -> list[i] == pCart)
		{
			printf("REMOVED\n");
			pList -> list[i] = NULL;
			return;
...
tick(tTrack * pTrack)
{
	...
	for(int i = 0; i < pList -> size; ++i)
	{
		...
		{
			printf("COLLISION!\nx:%u, y:%u\n", pDest -> cart -> cor.x, pDest -> cart -> cor.y);
			//
			/*
			*/
			pCart -> cor.x = EMPTY;
			pCart -> cor.y = EMPTY;
			pDest -> cart -> cor.x = EMPTY;
			pDest -> cart -> cor.y = EMPTY;
			remove_cart(pList, pCart);
			remove_cart(pList, pDest -> cart);
			pSrc -> cart = NULL;
			pDest -> cart = NULL;
			++collisions;
