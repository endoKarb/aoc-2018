...
scan_input(List * plist)
{
			...
			&id, &coor[0], &coor[1], &size[0], &size[1]) == 5)
	{
		...
		/*
		printf("id %u, coor %u %u, size %u %u\n",
				id, coor[0], coor[1], size[0], size[1]);


		printf("id %u, coor %u %u, size %u %u\n",
				pline->id, pline->coor[0], pline->coor[1], pline->size[0], pline->size[1]);
		*/
	}
}
...
void
update_map(Fabric * pmap, Line * pline, List * plist)
{
	...
	for(int i = voff; i < pline->size[1] + voff; ++i)
	{
		for(int j = hoff; j < pline->size[0] + hoff; ++j)
		{
			...
			{
				Line * old = plist->list[pmap->map[i][j] - 1];
